...
const CustomNodeTransferFrax = ({ id, data }) => {
  const amount = Number(data.amount);

  const truncatedAddress = data.address.length > 10 ? `${data.address.substring(0, 10)}...` : data.address;
  return (
    <div className="flex rounded-xl overflow-hidden shadow-xl bg-gradient-to-r from-purple-500 to-purple-700 border border-purple-800 w-80">
      <div className="flex items-center justify-center w-16 bg-purple-800">
      ...
      <Handle type="target" position={Position.Top} className="w-4 h-4 bg-purple-700" />
    </div>
  );
};

export default CustomNodeTransferFrax;
